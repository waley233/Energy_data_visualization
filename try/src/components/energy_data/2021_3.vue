...
import Data_2016_2 from './2021_3da.json'; // 引入JSON数据
import ecStat from 'echarts-stat';
import anotherData from './2021_3mv.json';
export default {
  name: 'Module2016_1',
  created() {
  ...
  methods: {
    ...
    myEcharts1() {
      ...
      var option;
      // 使用导入的JSON数据
      const timeData = Data_2016_2.map(item => {
        // 解析日期字符串为日期对象
        return new Date(item.Date).getTime(); // 转换为时间戳
      });
      const hvacActualData = Data_2016_2.map(item => [new Date(item.Date).getTime(), item['Power[kW]']]);
      // prettier-ignore
      const chillerPowerData = Data_2016_2.map(item => [new Date(item.Date).getTime(), item['PV panels power [kW]']]);
      option = {
        title: {
          text: 'Power and PV panels power and Battery system power ',
          left: 'center'
        },
        tooltip: {
          trigger: 'axis',
        ...
        legend: {
          data: ['Battery system power', 'Power and PV panels power'],
          left: 10
        },
        toolbox: {
          feature: {
        ...
        yAxis: [
          {
            name: 'Power and PV panels power [kW]',
            type: 'value',
            max: 5
          },
          {
            gridIndex: 1,
            name: 'Battery system power [kW]',
            type: 'value',
            inverse: true
          }
        ],
    ...
    myEcharts2() {
      ...
      const option = {
        title: {
          text: 'Power and PV panels power and Battery system power  vs Time',
          left: 'center'
        },
        tooltip: {
          trigger: 'axis',
        ...
        legend: {
          data: ['Power[kW]', 'PV panels power [kW]'],
          left: 10
        },
        toolbox: {
          feature: {
    ...
    myEcharts3() {
      ...
      var newData = Data_2016_2.map(item => [
        item['Power[kW]'],
        item['PV panels power [kW]']
      ]);
    ...
    myEcharts4() {
      ...
      var days = anotherData.map(item => item.Time);
      var values1 = anotherData.map(item => item['Power[kW]']);
      var values2 = anotherData.map(item => item['PV panels power [kW]']);

      option = {
        title: {
          text: 'Power and PV panels power (Variance)',
          left: 'left' // 将标题放置在左边
        },
        tooltip: {
          trigger: 'axis'
  ...
  mounted() {
    this.myEcharts1();
    this.myEcharts2(); // 直接调用，不需要async/await
    this.myEcharts3();
    this.myEcharts4()
  },
