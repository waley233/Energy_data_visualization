...
import Data_2016_2 from './2021_2da.json'; // 引入JSON数据
import ecStat from 'echarts-stat';
import anotherData from './2021_2mv.json';
export default {
  name: 'Module2016_1',
  created() {
  ...
  methods: {
    ...
    myEcharts1() {
      ...
      const hvacActualData = Data_2016_2.map(item => [new Date(item.Date).getTime(), item['Humidifier power [kW]']]);
      // prettier-ignore
      const chillerPowerData = Data_2016_2.map(item => [new Date(item.Date).getTime(), item['HV light Power [kW]']]);
      option = {
        title: {
          text: 'Humidifier power and HV light Power  ',
          left: 'center'
        },
        tooltip: {
          trigger: 'axis',
        ...
        legend: {
          data: ['HV light Power', 'Humidifier power'],
          left: 10
        },
        toolbox: {
          feature: {
        ...
        yAxis: [
          {
            name: 'Humidifier power [kW]',
            type: 'value',
            max: 5
          },
          {
            gridIndex: 1,
            name: 'HV light Power [kW]',
            type: 'value',
            inverse: true
          }
        ],
    ...
    myEcharts2() {
      ...
      const option = {
        title: {
          text: 'Humidifier power and HV light Power  vs Time',
          left: 'center'
        },
        tooltip: {
          trigger: 'axis',
        ...
        legend: {
          data: ['Humidifier power [kW]', 'HV light Power [kW]'],
          left: 10
        },
        toolbox: {
          feature: {
    ...
    myEcharts3() {
      ...
      var newData = Data_2016_2.map(item => [
        item['Humidifier power [kW]'],
        item['HV light Power [kW]']
      ]);
    ...
    myEcharts4() {
      ...
      var values1 = anotherData.map(item => item['Humidifier power [kW]']);
      var values2 = anotherData.map(item => item['HV light Power [kW]']);

      option = {
        title: {
          text: 'Humidifier power and HV light Power (Variance)',
          left: 'left' // 将标题放置在左边
        },
        tooltip: {
          trigger: 'axis'
