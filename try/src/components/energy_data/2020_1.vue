...
import Data_2016_1 from './2020_1da.json'; // 引入JSON数据
import ecStat from 'echarts-stat';
import anotherData from './2020_1mv.json';
...
export default {
  ...
  methods: {
    ...
    myEcharts1() {
      ...
      const hvacActualData = Data_2016_1.map(item => [new Date(item.Date).getTime(), item['HVAC Actual [kW]']]);
      // prettier-ignore
      const chillerPowerData = Data_2016_1.map(item => [new Date(item.Date).getTime(), item['Chiller Power [kW]']]);
      option = {
        title: {
